...
def l1_l2_loss(y_true, y_pred, l1_weight, l2_weight):
    ...
    if l1_weight != 0:
        loss += l1_weight * tf.keras.metrics.mean_absolute_error(y_true, y_pred)

    if l2_weight != 0:
        loss += l2_weight * tf.keras.metrics.mean_squared_error(y_true, y_pred)

    return loss


def compute_receptive_field_length(stacks, dilations, filter_length, target_field_length):
    #half_filter_length==1
    #滤波器的长度是3，利用了上下文信息
...
def keras_linear_to_ulaw(x, u=255.0):
    x = tf.keras.backend.sign(x) * (ktf.keras.backend.log(1 + u * tf.keras.backend.abs(x)) / tf.keras.backend.log(1 + u))
    return x
